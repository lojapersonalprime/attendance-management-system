...
export default async function EmployeesPage({ searchParams }: { searchParams: Promise<EmployeeListParams & { sucesso?: string; erro?: string }> }) {
  ...
  return (
    <>
      ...
      <form className="admin-filter-panel mb-5 rounded-[1.5rem] p-4 lg:p-5" method="get"><div className="mb-4 flex items-center justify-between gap-3"><p className="eyebrow text-[var(--primary)]">ENCONTRE RAPIDAMENTE</p><p className="text-xs text-[var(--muted-foreground)]">Filtros por cadastro e operação</p></div><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5"><label className="grid gap-1 text-sm font-medium xl:col-span-2">Buscar<input className="input" name="q" defaultValue={params.q} placeholder="Nome, matrícula ou código do relógio" /></label><label className="grid gap-1 text-sm font-medium">Status<select className="input" name="status" defaultValue={params.status ?? ""}><option value="">Todos</option>{employeeStatuses.map((status) => <option key={status} value={status}>{employeeStatusLabels[status]}</option>)}</select></label><label className="grid gap-1 text-sm font-medium">Unidade<select className="input" name="unitId" defaultValue={params.unitId ?? ""}><option value="">Todas</option>{activeOptions.units.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="grid gap-1 text-sm font-medium">Jornada<select className="input" name="scheduleTemplateId" defaultValue={params.scheduleTemplateId ?? ""}><option value="">Todas</option>{activeOptions.schedules.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div><details className="mt-4"><summary className="cursor-pointer text-sm font-semibold text-[var(--primary)]">Mais filtros</summary><div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4"><label className="grid gap-1 text-sm font-medium">Tipo de vínculo<select className="input" name="employmentType" defaultValue={params.employmentType ?? ""}><option value="">Todos</option>{employmentTypes.map((type) => <option key={type} value={type}>{employmentTypeLabels[type]}</option>)}</select></label><label className="grid gap-1 text-sm font-medium">Setor<select className="input" name="departmentId" defaultValue={params.departmentId ?? ""}><option value="">Todos</option>{activeOptions.departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="grid gap-1 text-sm font-medium">Cargo<select className="input" name="positionId" defaultValue={params.positionId ?? ""}><option value="">Todos</option>{activeOptions.positions.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="grid gap-1 text-sm font-medium">Tag<select className="input" name="tagId" defaultValue={params.tagId ?? ""}><option value="">Todas</option>{activeOptions.tags.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="grid gap-1 text-sm font-medium">Cadastro<select className="input" name="provisional" defaultValue={params.provisional ?? ""}><option value="">Todos</option><option value="true">Provisório</option><option value="false">Completo</option></select></label><label className="grid gap-1 text-sm font-medium">Código do relógio<input className="input" name="enNo" defaultValue={params.enNo} /></label></div></details><div className="mt-4 flex flex-wrap gap-2"><button className="min-h-11 rounded-xl bg-[var(--primary)] px-4 text-sm font-semibold text-[var(--primary-foreground)]" type="submit">Filtrar</button><Link className="inline-flex min-h-11 items-center rounded-xl border border-[var(--border)] px-4 text-sm font-semibold text-[var(--foreground)] hover:bg-[var(--surface-elevated)]" href={employeesRoute}>Limpar</Link></div></form>
