  ...
  return <>
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <div className="flex items-center gap-2">
        {removalPreview && removalPreview.mode !== "PRESERVE_ONLY" ? <EmployeeRemovalAction action={removeEmployeeAction} employeeId={employee.id} fullName={employee.fullName} hasMobileAccess={removalPreview.mobileAccessActive} mode={removalPreview.mode} /> : null}
    ...
    {tab === "resumo" ? <section className="space-y-5">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <Metric label="Horas trabalhadas" value={employee.dailySummaries[0] ? formatMinutes(employee.dailySummaries[0].workedMinutes) : "—"} tone="green" />
        <Metric label="Saldo" value={employee.dailySummaries[0] ? `${employee.dailySummaries[0].positiveMinutes - employee.dailySummaries[0].negativeMinutes >= 0 ? "+" : ""}${formatMinutes(employee.dailySummaries[0].positiveMinutes - employee.dailySummaries[0].negativeMinutes)}` : "—"} tone="blue" />
...
function EmployeeCalculationSection({ employee, canManage }: { employee: EmployeeRecord; canManage: boolean }) {
  ...
  return <section className="space-y-5">
    <article className="rounded-xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--primary)]">APURAÇÃO</p><h2 className="mt-1 text-lg font-semibold">Horas e marcações da competência</h2><p className="mt-1 text-sm text-[var(--muted-foreground)]">Calculado pelas marcações originais e pelo modelo vigente em cada data.</p></div><p className="text-xs text-[var(--muted-foreground)]">Os dados não são atualizados em tempo real.</p></div>{latest ? <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Horas previstas" value={formatMinutes(latest.expectedMinutes)} tone="blue" /><Metric label="Horas trabalhadas" value={formatMinutes(latest.validWorkedMinutes)} tone="green" /><Metric label="Saldo negativo" value={formatMinutes(latest.negativeMinutes)} tone="red" /><Metric label="Pendências" value={String(latest.inconsistencies.length)} tone="yellow" href={employeeRoute(employee.id, { aba: "pendencias" })} /></div> : null}</article>
