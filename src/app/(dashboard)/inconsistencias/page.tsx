...
import { MobileLocationReviewList } from "@/components/mobile-attendance/mobile-location-review-list";
import { PageHeader } from "@/components/layout/page-header";
import { StatusBadge } from "@/components/ui/status-badge";
import { executeBulkIssueActionAction, reviewMobileLocationIssueAction } from "@/app/(dashboard)/inconsistencias/actions";
...
export default async function InconsistenciesPage({ searchParams }: { searchParams: Promise<{ status?: string; severity?: string; type?: string; from?: string; until?: string; employee?: string; unit?: string; department?: string; schedule?: string; sucesso?: string; erro?: string }> }) {
  ...
  const mobileLocationTypes = new Set(["MOBILE_PUNCH_OUTSIDE_AUTHORIZED_AREA", "MOBILE_PUNCH_LOW_ACCURACY"]);
  const mobileLocationIssues = inconsistencies.filter((item) => mobileLocationTypes.has(item.type));
  const remainingIssues = inconsistencies.filter((item) => !mobileLocationTypes.has(item.type));
  return <>
    <PageHeader eyebrow="CAIXA DE TRABALHO" title="Pendências" description="Revise somente o que ainda precisa de uma decisão do RH." />
    ...
    <section className="surface mb-5 flex flex-wrap items-center justify-between gap-3 rounded-[1.5rem] p-5"><div><p className="eyebrow text-[var(--primary)]">FILA ATUAL</p><p className="numeric font-display mt-2 text-5xl font-semibold leading-none">{openCount}</p><p className="mt-2 text-sm text-[var(--muted-foreground)]">pendência(s) para revisar nos filtros atuais</p></div><StatusBadge tone={openCount > 0 ? "warning" : "success"}>{openCount > 0 ? "Ação necessária" : "Tudo em dia"}</StatusBadge></section>
    ...
    {canManage ? <MobileLocationReviewList action={reviewMobileLocationIssueAction} items={mobileLocationIssues.map((item) => { const presentation = getAttendanceIssuePresentation(item.type); return { id: item.id, employee: item.employee?.fullName ?? "Funcionário sem vínculo", date: item.date ? formatBusinessDate(item.date, "dd/MM/yyyy") : "Data não informada", title: presentation.title, description: item.description }; })} /> : null}
    {remainingIssues.length === 0 && mobileLocationIssues.length === 0 ? <p className="rounded-xl border bg-white p-6 text-sm text-[var(--muted-foreground)]">Nenhuma pendência para os filtros selecionados.</p> : remainingIssues.length > 0 ? <BulkIssueList canManage={canManage} executeAction={executeBulkIssueActionAction} totalFiltered={totalFiltered} items={remainingIssues.map((item) => { const presentation = getAttendanceIssuePresentation(item.type); return { id: item.id, type: item.type, title: presentation.title, employee: item.employee?.fullName ?? "Funcionário sem vínculo", employeeId: item.employeeId, date: item.date ? formatBusinessDate(item.date, "dd/MM/yyyy") : "Data não informada", businessDate: item.date?.toISOString().slice(0, 10) ?? null, description: presentation.description, impact: item.severity === "CRITICAL" ? "Requer correção antes do fechamento." : item.severity === "WARNING" ? "Requer revisão do RH." : "Aviso para acompanhamento.", status: getInconsistencyStatusLabel(item.status), severity: getSeverityLabel(item.severity), reviewHref: (item.dailySummary ? attendanceSummaryRoute(item.dailySummary.id) : "/inconsistencias") as Route }; })} /> : null}
  </>;
}
