...
export default async function AttendancePage({ searchParams }: { searchParams: Promise<{ reference?: string; employeeId?: string; employmentType?: string; calculationPolicyId?: string; scheduleTemplateId?: string; status?: string; inconsistency?: string; importFileId?: string }> }) {
  ...
  return <>
    ...
    <form className="mb-5 grid gap-2 rounded-lg border bg-white p-4 md:grid-cols-2 xl:grid-cols-4">
      ...
      <label className="grid gap-1 text-sm">Status<select className="input" name="status" defaultValue={query.status ?? ""}><option value="">Todos</option>{statuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label>
      <label className="grid gap-1 text-sm">Inconsistência<input className="input" name="inconsistency" placeholder="Ex.: MISSING_SCHEDULE" defaultValue={query.inconsistency ?? ""} /></label>
