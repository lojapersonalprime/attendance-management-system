...
export default async function AttendanceSummaryDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ sucesso?: string; erro?: string }> }) {
  ...
  return <>
    ...
    <section className="grid gap-4 rounded-xl border bg-white p-5 shadow-sm md:grid-cols-2 xl:grid-cols-5"><Metric label="Horas previstas" value={showsExpectedDuration ? formatMinutes(summary.expectedMinutes) : getCalculationPresentationLabel(presentationState)} /><Metric label="Horas trabalhadas" value={showsCalculatedDurations ? formatMinutes(summary.workedMinutes) : presentationState === "INCOMPLETE" ? `Parcial: ${formatMinutes(summary.recordedMinutes)}` : getCalculationPresentationLabel(presentationState)} /><Metric label="Saldo" value={showsCalculatedDurations ? <DurationDisplay minutes={balance} sign /> : "Indisponível"} tone={showsCalculatedDurations ? balance < 0 ? "danger" : balance > 0 ? "success" : "neutral" : "warning"} /><Metric label="Pendências" value={groupedActionableInconsistencies.length > 0 ? `${groupedActionableInconsistencies.length} para revisar` : "Nenhuma"} tone={groupedActionableInconsistencies.length > 0 ? "warning" : "success"} /><Metric label="Status" value={showsCalculatedDurations ? getCalculationPresentationLabel(presentationState) : getCalculationPresentationLabel(presentationState)} /></section>
    <section className="mt-5 grid gap-5 rounded-xl border bg-white p-5 shadow-sm lg:grid-cols-[0.8fr_1.2fr]"><div><h2 className="text-lg font-semibold">Linha do tempo do dia</h2><p className="mt-1 text-sm text-[var(--muted-foreground)]">Batidas registradas no arquivo importado.</p><div className="mt-5"><AttendanceTimeline punches={punches} technical={false} /></div></div><div className="grid content-start gap-3 text-sm sm:grid-cols-2"><Info label="Jornada prevista" value={summary.scheduleAssignment?.scheduleTemplate.name ?? "Aguardando configuração"} /><Info label="Primeiro período" value={workPeriod(summary.calculationMemory, 0)} /><Info label="Segundo período" value={workPeriod(summary.calculationMemory, 1)} /><Info label="Intervalo" value={summary.breakMinutes > 0 ? formatMinutes(summary.breakMinutes) : "Não se aplica"} /><Info label="Atraso" value={formatMinutes(summary.lateMinutes)} /><Info label="Saída antecipada" value={formatMinutes(summary.earlyDepartureMinutes)} /><Info label="Excedente pendente" value={formatMinutes(summary.pendingExcessMinutes)} /><Info label="Saldo calculado" value={showsCalculatedDurations ? <DurationDisplay minutes={balance} sign /> : "Indisponível"} /></div></section>
