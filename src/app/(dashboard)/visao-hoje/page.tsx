...
export default async function UnitTodayPage({ searchParams }: { searchParams: Promise<{ unitId?: string }> }) {
  ...
  return <><PageHeader title={`${selectedUnit?.name ?? "Unidade"} — Hoje`} description="Situação com base na última batida registrada; não representa localização atual." />{canView ? <form className="mb-5 flex max-w-md gap-2"><select className="input" defaultValue={selectedUnitId ?? ""} name="unitId" onChange={undefined}><option value="">Selecione uma unidade</option>{units.map((unit) => <option key={unit.id} value={unit.id}>{unit.name}</option>)}</select><button className="rounded-md border px-4 py-2 text-sm font-semibold" type="submit">Ver</button></form> : null}<section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{employees.map((employee) => { const summary = employee.dailySummaries[0]; const timeline = getCalculatedTimeline(summary?.calculationMemory, summary?.recordedMinutes ?? 0).punches; const last = timeline.at(-1); const uncalculated = employee.mobilePunches[0] ?? employee.deviceLinks.flatMap((link) => link.rawPunches)[0]; const description = last ? `${punchPresentation[last.punchCode].state} desde ${formatInTimeZone(last.occurredAt, "America/Fortaleza", "HH:mm")}` : uncalculated ? `Batida registrada às ${formatInTimeZone("registeredAt" in uncalculated ? uncalculated.registeredAt : uncalculated.occurredAt, "America/Fortaleza", "HH:mm")}; aguardando apuração` : "Ainda sem registro hoje"; return <article className="rounded-xl border bg-white p-5 shadow-sm" key={employee.id}><h2 className="font-semibold">{employee.fullName}</h2><p className="mt-2 text-sm text-[var(--muted-foreground)]">{description}</p>{summary ? <Link className="mt-4 inline-flex text-sm font-semibold text-[var(--primary)]" href={`/apuracao/${summary.id}`}>Ver registro do ponto</Link> : null}</article>; })}{employees.length === 0 ? <p className="rounded-xl border bg-white p-5 text-sm text-[var(--muted-foreground)]">Não há funcionários ativos nesta unidade.</p> : null}</section></>;
}
