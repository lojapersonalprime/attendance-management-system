...
export function AttendanceTimeline({ punches, technical = true, waitingForRecalculation = false }: { punches: Array<{ id?: string; occurredAt: Date; punchCode: ImportedPunchCode; origin?: "RAW_PUNCH" | "MANUAL_ADJUSTMENT"; adjustmentId?: string; reason?: string }>; technical?: boolean; waitingForRecalculation?: boolean }) {
  ...
  return <div className="relative space-y-4 before:absolute before:bottom-4 before:left-4 before:top-4 before:w-px before:bg-slate-200">
    {[...punches].sort((left, right) => left.occurredAt.getTime() - right.occurredAt.getTime()).map((punch) => {
      ...
      return <div className="relative flex items-center gap-3" key={punch.id ?? `${punch.occurredAt.toISOString()}-${punch.punchCode}`}>
        ...
        <div className="min-w-0"><p className="font-semibold text-slate-950">{formatInTimeZone(punch.occurredAt, BUSINESS_TIME_ZONE, "HH:mm")}</p><p className="text-sm text-[var(--muted-foreground)]">{item.label}{punch.origin === "MANUAL_ADJUSTMENT" ? " · ajuste do RH" : ""}</p>{punch.origin === "MANUAL_ADJUSTMENT" && punch.reason ? <p className="text-xs text-[var(--muted-foreground)]">{punch.reason}</p> : null}</div>
