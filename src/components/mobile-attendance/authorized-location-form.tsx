...
import { ErrorState, InlineSpinner, LoadingButton } from "@/components/ui/async-feedback";
import { PLACE_SEARCH_DEBOUNCE_MS, shouldSearchPlaces, type PlaceDetails, type PlaceSuggestion } from "@/modules/places/domain/place-search";
...
function mapUrl(selection: Selection) {
  return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${selection.latitude},${selection.longitude}`)}`;
}
...
export function AuthorizedLocationForm({ action, units, location }: { action: ServerAction; units: UnitOption[]; location?: LocationDraft }) {
  const initialSelection: Selection | undefined = location ? {
    method: location.placeProvider === "GOOGLE_PLACES" && location.providerPlaceId ? "PLACE" : "MANUAL",
    latitude: String(location.latitude),
    longitude: String(location.longitude),
    formattedAddress: location.formattedAddress ?? "",
    placeProvider: location.placeProvider === "GOOGLE_PLACES" ? "GOOGLE_PLACES" : undefined,
  ...
  async function chooseSuggestion(suggestion: PlaceSuggestion) {
    ...
    try {
      const token = sessionToken.current ?? startSession();
      const response = await fetch(`/api/place-search/${encodeURIComponent(suggestion.providerPlaceId)}?${new URLSearchParams({ sessionToken: token })}`);
      ...
      setSelection({ method: "PLACE", placeProvider: place.provider, providerPlaceId: place.providerPlaceId, formattedAddress: place.formattedAddress, latitude: String(place.latitude), longitude: String(place.longitude) });
  ...
  return <form action={action} className="grid gap-4 rounded-xl border bg-white p-5 shadow-sm">
