...
export function EmployeeBulkList({ action, employees, units, departments, positions, tags, schedules }: {
  ...
  const [bulkAction, setBulkAction] = useState<BulkAction>("EMPLOYMENT_TYPE");
  ...
  return (
    <form action={action} className="space-y-4">
      <section className="grid gap-3 rounded-lg border bg-white p-4 lg:grid-cols-[1fr_1fr_1fr_auto] lg:items-end">
        <label className="grid gap-1 text-sm font-medium">Ação em lote<select className="input" name="bulkAction" value={bulkAction} onChange={(event) => setBulkAction(event.target.value as BulkAction)}>{(Object.keys(actionLabels) as BulkAction[]).map((item) => <option key={item} value={item}>{actionLabels[item]}</option>)}</select></label>
      ...
      <div className="overflow-x-auto rounded-lg border bg-white">
        <table className="w-full min-w-[1250px] text-left text-sm"><thead className="border-b text-xs uppercase tracking-wide text-[var(--muted-foreground)]"><tr><th className="px-3 py-3"><input aria-label="Selecionar todos desta página" type="checkbox" checked={allSelected} onChange={toggleAll} /></th><th className="px-4 py-3">Nome</th><th className="px-4 py-3">EnNo</th><th className="px-4 py-3">Vínculo</th><th className="px-4 py-3">Unidade</th><th className="px-4 py-3">Setor</th><th className="px-4 py-3">Cargo</th><th className="px-4 py-3">Jornada atual</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Cadastro</th><th className="px-4 py-3">Última marcação</th><th className="px-4 py-3">Ações</th></tr></thead><tbody>{employees.map((employee) => {
          ...
          return <tr className="border-b last:border-0" key={employee.id}><td className="px-3 py-3"><input aria-label={`Selecionar ${employee.fullName}`} type="checkbox" name="employeeIds" value={employee.id} checked={selected.has(employee.id)} onChange={() => toggle(employee.id)} /></td><td className="px-4 py-3"><p className="font-medium">{employee.fullName}</p><p className="text-xs text-[var(--muted-foreground)]">{employee.registration ?? "Sem matrícula"}</p></td><td className="px-4 py-3">{employee.deviceLinks.filter((link) => link.active).map((link) => link.externalEmployeeNumber).join(", ") || "—"}</td><td className="px-4 py-3">{employmentTypeLabels[employee.employmentType]}</td><td className="px-4 py-3">{employee.unit?.name ?? "—"}</td><td className="px-4 py-3">{employee.department?.name ?? "—"}</td><td className="px-4 py-3">{employee.position?.name ?? "—"}</td><td className="px-4 py-3">{employee.scheduleAssignments[0]?.scheduleTemplate.name ?? "Pendente"}</td><td className="px-4 py-3">{employeeStatusLabels[employee.status]}</td><td className="px-4 py-3">{employee.provisional ? <span className="rounded-full bg-amber-100 px-2 py-1 text-xs text-amber-900">Provisório</span> : <span className="rounded-full bg-emerald-100 px-2 py-1 text-xs text-emerald-900">Completo</span>}</td><td className="px-4 py-3">{lastPunch ? new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short", timeZone: "America/Fortaleza" }).format(lastPunch.occurredAt) : "—"}</td><td className="px-4 py-3"><Link className="font-semibold text-[var(--primary)] hover:underline" href={employeeRoute(employee.id)}>Visualizar</Link></td></tr>;
        })}</tbody></table>
      </div>
    </form>
  );
