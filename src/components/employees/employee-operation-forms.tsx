...
export function EmploymentPolicyForm({ action, employeeId, employmentType, policies }: { action: ServerAction; employeeId: string; employmentType: string; policies: NamedOption[] }) {
  return <form action={action} className="grid gap-3 md:grid-cols-2 xl:grid-cols-3"><input name="employeeId" type="hidden" value={employeeId} /><label className="grid gap-1 text-sm font-medium">Tipo de vínculo<select className="input" defaultValue={employmentType} name="employmentType"><option value="EMPLOYEE">CLT/Funcionário</option><option value="INTERN">Estagiário</option><option value="APPRENTICE">Jovem aprendiz</option><option value="CONTRACTOR">PJ/Prestador</option><option value="OTHER">Outro</option></select></label><label className="grid gap-1 text-sm font-medium">Regra de cálculo<select className="input" name="calculationPolicyId"><option value="">Selecione</option>{policies.map((policy) => <option key={policy.id} value={policy.id}>{policy.name}</option>)}</select></label><label className="grid gap-1 text-sm font-medium">Início<input className="input" name="validFrom" type="date" /></label><label className="grid gap-1 text-sm font-medium">Fim (opcional)<input className="input" name="validUntil" type="date" /></label><label className="grid gap-1 text-sm font-medium md:col-span-2">Motivo<input className="input" name="reason" placeholder="Ex.: alteração contratual confirmada pelo RH" /></label><label className="flex items-center gap-2 text-sm"><input name="closePrevious" type="checkbox" />Encerrar período anterior</label><label className="flex items-center gap-2 text-sm"><input name="retroactiveConfirmed" type="checkbox" />Confirmo a alteração retroativa</label><OperationHint steps={["Validando período…", "Salvando configuração…", "Recalculando o período afetado…"]} /><LoadingButton loadingLabel="Salvando configuração…">Salvar configuração avançada</LoadingButton></form>;
}
...
export function ScheduleAssignmentForm({ action, employeeId, schedules, currentScheduleName }: { action: ServerAction; employeeId: string; schedules: NamedOption[]; currentScheduleName?: string }) {
  const assigning = !currentScheduleName;
  return <form action={action} className="grid gap-3 rounded-xl border bg-white p-5 shadow-sm md:grid-cols-2 xl:grid-cols-3"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--primary)] md:col-span-2 xl:col-span-3">{assigning ? "MODELO NÃO ATRIBUÍDO" : "Alterar configuração"}</p><h2 className="md:col-span-2 xl:col-span-3 text-lg font-semibold">{assigning ? "Atribuir modelo de horário" : "Trocar modelo de horário"}</h2><p className="md:col-span-2 xl:col-span-3 text-sm text-[var(--muted-foreground)]">{assigning ? "Sem modelo de horário, o sistema não inventa horas previstas. Escolha um modelo para recalcular a apuração do período." : "A nova vigência preserva o histórico e recalcula automaticamente os dias abertos e cobertos."}</p><input name="employeeId" type="hidden" value={employeeId} /><label className="grid gap-1 text-sm font-medium">Novo modelo<select className="input" name="scheduleTemplateId"><option value="">Selecione</option>{schedules.map((schedule) => <option key={schedule.id} value={schedule.id}>{schedule.name}</option>)}</select></label><label className="grid gap-1 text-sm font-medium">Início da vigência<input className="input" name="validFrom" required type="date" /></label><label className="grid gap-1 text-sm font-medium">Fim (opcional)<input className="input" name="validUntil" type="date" /></label><label className="flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2 text-sm"><input name="closePrevious" type="checkbox" />Encerrar o modelo atual no dia anterior</label><label className="flex items-center gap-2 text-sm"><input name="retroactiveConfirmed" type="checkbox" />Confirmo a alteração retroativa</label><label className="grid gap-1 text-sm font-medium md:col-span-2">Motivo<input className="input" name="reason" placeholder="Ex.: horário da manhã confirmado pelo RH" /></label><div className="rounded-lg bg-orange-50 p-3 text-sm text-orange-950 md:col-span-2 xl:col-span-3"><p className="font-semibold">Impacto previsto</p><p className="mt-1">Modelo anterior: {currentScheduleName ?? "Sem modelo de horário"}. Meses fechados e dias sem cobertura continuam protegidos.</p></div><OperationHint steps={["Validando período…", "Salvando modelo de horário…", "Recalculando registros…"]} /><LoadingButton loadingLabel="Salvando modelo e recalculando…">{assigning ? "Atribuir modelo e recalcular" : "Salvar modelo e recalcular"}</LoadingButton></form>;
}
