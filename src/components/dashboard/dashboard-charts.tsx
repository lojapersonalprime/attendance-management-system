...
export function DashboardCharts({ dailyHours, pendingCategories, balanceTrend }: DashboardChartsProps) {
  return <div className="grid gap-5 xl:grid-cols-2">
    ...
    <ChartCard title="Evolução do saldo" description="Débitos e excedentes pendentes acumulados por dia." empty={balanceTrend.length === 0} summary={balanceTrend.length ? "A linha vermelha representa saldo negativo e a linha amarela, excedentes pendentes." : "Não há saldo calculado nesta competência."}>
      <ResponsiveContainer width="100%" height="100%"><LineChart data={balanceTrend} margin={{ left: 8, right: 8 }}><CartesianGrid stroke="#34363c" strokeDasharray="3 3" vertical={false} /><XAxis dataKey="day" tick={{ fill: "#A1A1AA", fontSize: 12 }} axisLine={false} tickLine={false} /><YAxis tickFormatter={(value) => formatMinutes(Number(value))} tick={{ fill: "#A1A1AA", fontSize: 12 }} axisLine={false} tickLine={false} width={58} /><Tooltip content={<TooltipDuration />} /><Legend wrapperStyle={{ color: "#A1A1AA", fontSize: 12 }} /><Line type="monotone" dataKey="negativeMinutes" name="Saldo negativo" stroke="#EF4444" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="pendingExcessMinutes" name="Excedente pendente" stroke="#F59E0B" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer>
    </ChartCard>
  </div>;
}
