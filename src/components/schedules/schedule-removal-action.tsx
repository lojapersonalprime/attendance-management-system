...
export function ScheduleRemovalAction({
  ...
  return <><button className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-red-200 px-3 text-sm font-semibold text-red-700 hover:bg-red-50" onClick={() => dialog.current?.showModal()} type="button"><Trash2 aria-hidden="true" size={16} />Excluir</button>
    <dialog aria-labelledby="schedule-removal-title" className="m-auto w-[min(94vw,34rem)] rounded-[1.25rem] border p-0 shadow-2xl backdrop:bg-slate-950/65" ref={dialog}>
      <form action={action} className="grid gap-5 p-5 sm:p-6">
        ...
        <div>
          ...
          <h2 className="font-display mt-2 text-3xl font-semibold leading-none" id="schedule-removal-title">Excluir modelo de horário?</h2>
          <p className="mt-3 text-sm leading-6 text-[var(--muted-foreground)]">O modelo <strong>{scheduleName}</strong> deixará de aparecer nas opções do RH. As jornadas e resumos históricos serão preservados.</p>
